...
@task
@require_settings
def setup_db(django='y', sample='n'):
...
@require_settings
def seed_db_sample():   
    execute(env.db.seed, sample='y')      

@task
@require_settings                    
def setup(django='y', sample='n'):
    """Setup new application deployment.  Run only once per project."""    
...
def deploystatic(django='y', fnpattern='[!.]*'):
